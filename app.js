...
// ==========================================
// 5. DYNAMIC OWNCAST CHAT IFRAME FLEX FIX
// ==========================================
function initOwncastIframeFix() {
  ...
  function injectIframeStyles() {
    try {
      ...
      let style = doc.getElementById('akmov-chat-fix');
      if (!style) {
        style = doc.createElement('style');
        style.id = 'akmov-chat-fix';
        doc.head.appendChild(style);
      }
      style.textContent = `
        html, body, #root, #app, [class*="App"], [class*="layout"], [class*="container"] {
        ...
        #chatroom-messages, [data-testid="chatroom-messages"], [class*="messages"], [class*="Messages"] {
          flex: 1 1 auto !important;
          min-height: 0 !important;
        }
        #chatroom-footer, [class*="footer"], [class*="Footer"], [class*="input-wrapper"] {
          margin-top: auto !important;
        }
      `;
    } catch (e) {
      // Security isolation fallback handled via Owncast Admin Custom CSS
    }
  }

  iframe.addEventListener('load', injectIframeStyles);
  injectIframeStyles();
  setInterval(injectIframeStyles, 3000);
}

initOwncastIframeFix();
