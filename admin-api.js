...
// ─── USER ROLES & PERMISSIONS PERSISTENCE ───────────────────────
const USERS_FILE = path.join(__dirname, 'users.json');

// Initialize users.json with default superadmin if it doesn't exist
if (!fs.existsSync(USERS_FILE)) {
  const initialUsers = [
    {
      email: 'akmovmedia@gmail.com',
      role: 'superadmin',
...
app.post('/api/users', (req, res) => {
  try {
    const { email, role, allowed_tabs } = req.body;
    if (!email) {
    ...
    const userObj = {
      email,
      role: role || 'locutor',
      allowed_tabs: allowed_tabs || ['pauta'],
      created_at: existingIndex >= 0 ? users[existingIndex].created_at : new Date().toISOString()
    };
    if (existingIndex >= 0) {
    ...
    console.log(`[Users] Usuario guardado: ${email} (${role})`);
...
app.delete('/api/users/:email', (req, res) => {
  try {
    ...
    if (email === 'akmovmedia@gmail.com') {
      return res.status(400).json({ success: false, error: 'No se puede eliminar el superadministrador principal' });
    }
    let users = [];
    if (fs.existsSync(USERS_FILE)) {
      users = JSON.parse(fs.readFileSync(USERS_FILE, 'utf8'));
    ...
    fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2));
    console.log(`[Users] Usuario eliminado: ${email}`);
    res.json({ success: true });
  } catch(err) {
    res.status(500).json({ success: false, error: err.message });
  }
