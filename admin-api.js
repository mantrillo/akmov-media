...
// ─── MENSAJE Y COMANDOS DE REDES SOCIALES EN OWNCAST ────────────
const SOCIALS_MESSAGE = `**AKMOV MEDIA • Redes Oficiales**
Instagram: [instagram.com/akmovmedia](https://instagram.com/akmovmedia)
YouTube: [youtube.com/@AKMOVMEDIA](https://youtube.com/@AKMOVMEDIA)
TikTok: [tiktok.com/@akmovmedia](https://tiktok.com/@akmovmedia)
Twitch: [twitch.tv/akmovmedia](https://twitch.tv/akmovmedia)
Kick: [kick.com/akmovmedia](https://kick.com/akmovmedia)
Discord: [discord.gg/pMK9CGM5qy](https://discord.gg/pMK9CGM5qy)
Web: [akmovmedia.com](https://akmovmedia.com)`;

/**
 * Publica un mensaje del sistema/bot oficial en el chat de Owncast
 */
async function sendSystemBroadcast(bodyText) {
  const candidateUrls = [
    process.env.OWNCAST_API_URL || 'http://localhost:8080',
    'http://127.0.0.1:8080',
  ...
  for (const baseUrl of candidateUrls) {
    try {
      ...
      const response = await fetch(`${cleanBase}/api/integrations/chat/send`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${OWNCAST_ADMIN_TOKEN}`
      ...
      if (response.ok) {
        console.log(`[Owncast Bot Broadcast] Mensaje enviado correctamente vía ${cleanBase}.`);
        return true;
      } else {
        console.warn(`[Owncast Bot Broadcast] ${cleanBase} respondió status ${response.status}: ${await response.text()}`);
      }
    } catch (error) {
      // Probar siguiente candidato
    }
  }
  console.error('[Owncast Bot Broadcast Error] No se pudo enviar el mensaje a ninguna URL de Owncast.');
  return false;
}

// ─── ENDPOINT UNIFICADO PARA WEBHOOKS DE OWNCAST ─────────────────
// Soporta tanto /owncast-webhook unificado como /chat, /nuevousuario, /cambiodenombre
app.post(['/owncast-webhook', '/chat', '/nuevousuario', '/cambiodenombre'], async (req, res) => {
  const { type, eventData } = req.body;
  const eventType = (type || '').toUpperCase();

  // 1. EVENTO: MENSAJES DE CHAT (CHAT / CHAT_MESSAGE / USER_CHAT)
  if (eventType === 'CHAT' || eventType === 'CHAT_MESSAGE' || eventType === 'USER_CHAT' || (!type && (req.body.body || req.body.text || (eventData && eventData.body)))) {
    const rawBody = (eventData && (eventData.body || eventData.rawBody || eventData.message)) ? (eventData.body || eventData.rawBody || eventData.message) : (req.body.body || req.body.text || '');
    const rawUser = (eventData && eventData.user) ? (eventData.user.displayName || eventData.user.name || eventData.user.id) : (req.body.user || req.body.username || 'Usuario');
    
    if (rawBody) {
      // Limpiar etiquetas HTML de Owncast (ej: "<p>!redes</p>" -> "!redes")
      const cleanBody = rawBody.replace(/<[^>]*>?/gm, '').trim();
      const userMessage = cleanBody.toLowerCase();

      console.log(`[Owncast Webhook Recibido] @${rawUser}: "${cleanBody}" (Tipo evento: ${eventType})`);

      // Comando !redes / !rrss / !sociales
      if (userMessage === '!redes' || userMessage === '!rrss' || userMessage === '!sociales' || userMessage.startsWith('!redes') || userMessage.startsWith('!rrss')) {
        console.log(`[Owncast Chat] ¡Comando ${userMessage} detectado! Enviando difusión de redes...`);
        await sendSystemBroadcast(SOCIALS_MESSAGE);
      }

      // Reenviar a clientes de overlay en tiempo real (SSE)
      ...
      alertClients.forEach(client => {
        try { client.write(`data: ${chatPayload}\n\n`); } catch(e) {}
      });
    }
  }

  // 2. EVENTO: NUEVO ESPECTADOR / USUARIO SE UNIÓ
  else if (eventType === 'USER_JOINED' || eventType === 'USER_JOIN' || req.path === '/nuevousuario') {
    const userName = (eventData && eventData.user) ? (eventData.user.displayName || eventData.user.id) : (req.body.user || 'Nuevo Espectador');
    ...
    alertClients.forEach(client => {
      try { client.write(`data: ${alertPayload}\n\n`); } catch(e) {}
    });
    console.log(`[Owncast Alert] Nuevo Usuario Unido: @${userName}`);
  }

  // 3. EVENTO: CAMBIO DE NOMBRE DE USUARIO
  else if (eventType === 'NAME_CHANGE' || eventType === 'NAME_CHANGED' || eventType === 'USER_NAME_CHANGED' || req.path === '/cambiodenombre') {
    const userName = (eventData && eventData.user) ? (eventData.user.displayName || eventData.user.id) : (req.body.user || 'Usuario');
    const oldName = req.body.oldName || (eventData && eventData.oldName) || '';
    const alertPayload = JSON.stringify({
      dataType: 'ALERT',
      type: 'NAME_CHANGE',
      user: userName,
    ...
    console.log(`[Owncast Alert] Cambio de Nombre: @${userName}`);
  }

  // Responder 200 OK inmediatamente a Owncast
  res.json({ success: true });
});


// ─── TEMPORIZADOR: DIFUSIÓN DE REDES CADA 30 MINUTOS ────────────
setInterval(async () => {
  try {
    // Verificar si Owncast está activo antes de enviar el mensaje automático
    const statusRes = await fetch(`${OWNCAST_API_URL}/api/status`);
    if (statusRes.ok) {
      const statusData = await statusRes.json();
      if (statusData.online) {
        console.log('[Auto-Socials] Enviando recordatorio de redes al chat de Owncast...');
        await sendSystemBroadcast(SOCIALS_MESSAGE);
      }
    }
  } catch (e) {
    // Si Owncast no está respondiendo, ignorar silenciosamente
  }
