/**
 * RUEDAS Y CANCHA OVERLAY ENGINE (app.js)
...
(function () {
  ...
  const DEFAULT_CONFIG = {
    streamerName: 'RUEDAS Y CANCHA',
    subtitle: 'DEPORTES • MOTORES • VALLE DEL HUASCO',
    tickerText: 'RUEDAS Y CANCHA • PASIÓN TUERCA • RALLY • FÚTBOL • VALLENAR • HUASCO • FREIRINA • ALTO DEL CARMEN',
    statusStarting: 'YA COMENZAMOS',
    statusEnding: '¡NOS VEMOS EN LA PRÓXIMA FECHA!',
    hostName: 'RUEDAS Y CANCHA',
  ...
  class RuedasEngine {
    ...
    applyThemeToCSS() {
      ...
      if (!root) return;
      const primary = this.config.theme?.neonPrimary || '#ff003c';
      const secondary = this.config.theme?.neonSecondary || '#ffe600';
    ...
    renderHeroLogo(containerEl) {
      ...
      const imgSrc = this.config.logoImageBase64 || this.config.logoImageUrl;
      ...
      if (this.config.logoType === 'image' && imgSrc) {
        ...
        containerEl.innerHTML = `<span style="font-family: var(--font-display); font-size: 72px; font-weight: 900; color: #fff; letter-spacing: 4px; text-shadow: 0 0 20px var(--neon-primary);">${this.config.streamerName || 'RUEDAS Y CANCHA'}</span>`;
      }
    }
    ...
    renderSocialBar(containerEl) {
      ...
      wrap.className = 'racing-social-bar';
      ...
      Object.keys(socials).forEach((key) => {
        ...
        if (item && item.enabled && item.handle) {
          ...
          el.className = 'racing-social-item';
